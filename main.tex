...
\begin{itemize}
  \item \textbf{RQ1 (Architecture):} How do MCP's core primitives (Tools,
    Resources, Prompts) diverge from the ``Agent Card'' and capability
    negotiation models of A2A and ACP?
  \item \textbf{RQ2 (Security \& State):} How does MCP's stateful JSON-RPC
    session model impact security boundaries compared to stateless
    REST-based function calling~\cite{hou:mcpsecurity2025}?
  \item \textbf{RQ3 (Convergence):} Can these protocols coexist? Is there
    evidence for a layered ``TCP/IP moment'' for the internet of agents?
\end{itemize}
...
\begin{itemize}
  \item \textbf{Layer 3 --- Collaboration} (``The Social Layer''):
    ...
    goals (``Plan a trip'')~\cite{google:a2a2024,ibm:acp2024}.
