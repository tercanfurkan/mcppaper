...
> *Note on sources: given the pace of development in this field, this section draws on both peer-reviewed literature and grey literature (practitioner blog posts and technical reports) following the Multivocal Literature Review (MLR) methodology for software engineering research [15].*
...
**Agent-to-UI rendering.** A2UI refers to the translation of agent output into structured UI components — cards, tables, code blocks — for human consumption. As the newest and least formalised layer of the stack (Google, December 2025, currently at v0.8 stable), A2UI has received almost no attention in the academic evaluation literature [14]. Unlike RAG or chatbot evaluation, A2UI output is visual and structured; standard NLP metrics do not apply directly. Practitioners have identified cross-layer observability — the ability to trace a user request coherently through A2UI, A2A, and MCP — as a critical unsolved structural gap in current stacks [14]. Our layer-wise fidelity scoring protocol directly addresses the measurement dimension of this gap. We operationalise R₂ by scoring the serialised JSON payload that drives the UI rendering, and discuss multimodal scoring via GPT-4o Vision as a stretch goal (Section 6.6).
...
Our work is most directly comparable to AgentMaster [6], which jointly employs A2A and MCP and evaluates with BERTScore and LLM-as-a-Judge. The key distinction is evaluation scope: AgentMaster measures end-to-end performance; we measure layer-wise fidelity. The CA-MCP system [4] demonstrates that augmenting the MCP layer with shared context stores can reduce LLM calls and improve robustness, but evaluates only aggregate task performance. The surveys of Yang et al. [1] and Ehtesham et al. [3] identify evaluation benchmarking as a critical gap in the agent protocol literature, explicitly noting that current efforts focus on task success and latency rather than communication efficiency or information fidelity. Our protocol directly addresses this gap.
